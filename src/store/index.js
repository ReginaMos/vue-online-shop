...
export default new Vuex.Store({
    ...
    getters: {
        ...
        SUM_CART(state) {
            return state.cart.reduce((acc, cur) => {
                return acc + (cur.count * cur.price);}, 0)
        }
    },

    mutations: {
        SET_TO_CART(state, product) {
            let elem = state.cart.find(user => user.id === product.id);
            if(elem) {
                elem.count += 1;
            }
            else {
        ...
        SET_COUNT(state, changer) {
            let index = state.cart.findIndex(user => user.id === changer.id);
            state.cart[index].count += changer.count; 
        },

        DELETE_ELEM(state, id) {
            let index = state.cart.findIndex(user => user.id === id);
            state.cart.splice(index, 1);
        },

        CHANGE_FILTER(state, filter) {
            state.headerFilter = filter;
        },

        CHANGE_HEART(state, id) {
          state.products.find(user => {
            if (user.id === id) {
              user.fav = !user.fav;
            }
          });
    ...
    actions: {
        ...
        UPDATE_COUNT({commit}, changer) {
            commit('SET_COUNT', changer);
        },

        DELETE_ELEM({commit}, id) {
            commit('DELETE_ELEM', id);
        },

        CHANGE_HEART({commit}, id) {
            commit('CHANGE_HEART', id);
        }
    },
